...
export function ArcChatbot({
  ...
  useEffect(() => {
    const roleGreetings: Record<string, string> = {
      ...
      teacher:
        "your classrooms, student attendance, timetables, or upcoming events",
        ...
        <div className="fixed bottom-6 right-6 z-50 w-80 sm:w-96 flex flex-col rounded-xl overflow-hidden shadow-2xl border border-zinc-700 bg-zinc-900">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 max-h-72 bg-zinc-900">
            {messages.map((m) => (
              ...
              >
                <div
                  className={`max-w-[80%] rounded-lg px-3 py-2 text-sm ${m.role === "user" ? "bg-[#e78a53] text-white" : "bg-zinc-800 text-zinc-200 border border-zinc-700"}`}
                >
                  {m.content}
                  {m.source && m.source !== "general" && (
          ...
          <div className="flex gap-2 p-3 bg-zinc-800 border-t border-zinc-700">
            <Input
              ...
              className="bg-zinc-700 border-zinc-600 text-white text-sm placeholder:text-zinc-400"
            />
            <Button
              size="icon"
              onClick={send}
              disabled={loading}
              className="bg-[#e78a53] hover:bg-[#d4784a] shrink-0"
            >
