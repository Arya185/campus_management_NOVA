...
export function fallbackResponse(type: string, data: any): any {
    ...
    for (let i = 0; i < classes.length; i++) {
      for (let j = i + 1; j < classes.length; j++) {
        ...
        if (
          ...
          conflicts.push({
            conflict: true,
            subjects: [a.subject || a.name, b.subject || b.name],
            day: a.day,
            overlap: `${a.start}–${b.end}`,
            message: `"${a.subject || a.name}" and "${b.subject || b.name}" overlap on ${a.day}.`,
          });
        }
      }
    }
  ...
  if (type === "anomaly") {
    ...
    const anomalies = records
      .map((r: any) => {
        const attended = r.attendance.filter(Boolean).length;
        const pct = Math.round((attended / r.attendance.length) * 100);
        return { studentId: r.studentId, name: r.name, attendancePct: pct };
      })
      .filter((r: any) => r.attendancePct < 75)
      .map((r: any) => ({
        ...r,
        alert: true,
        message: `${r.name} has only ${r.attendancePct}% attendance — below the 75% threshold.`,
      }));
    return {
  ...
  if (type === "chat") {
    ...
    if (
      ...
      return {
        reply:
          "Your timetable shows your scheduled classes for the week. Head to the Timetable section in your dashboard to view your full schedule and room details.",
        source: "timetable",
      };
    if (
      ...
      return {
        reply:
          "You can check your attendance percentage in the Attendance section. Make sure to maintain at least 75% attendance to avoid being detained.",
        source: "attendance",
      };
    if (
      q.includes("canteen") ||
    ...
    if (
      ...
      return {
        reply:
          "Visit the Internships section for the latest placement drives, internship listings, and application deadlines.",
        source: "general",
      };
    return {
      reply:
